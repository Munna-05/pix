            ...
            <div className=' container py-16 bg-[]  mx-auto grid lg:grid-cols-6 md:grid-cols-4 grid-cols-2 px-3 gap-1'>
                {
                    images?.map((res, i) => {
                        return (
                            <motion.div onClick={()=>navigate(`/details/${res?._id}/${res.userId}`)} className='cursor-pointer' initial={{opacity:0,scale:0.7}} animate={{opacity:1,scale:1,transition:{delay:i/9,type:'spring'}}}>
